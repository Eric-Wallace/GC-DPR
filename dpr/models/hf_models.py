...
from transformers import BertConfig, BertModel, EsmModel, AutoConfig
from transformers.optimization import AdamW
from transformers import BertTokenizer, RobertaTokenizer, EsmTokenizer
...
def get_esm_biencoder_components(args, inference_only: bool = False, **kwargs):
    dropout = args.dropout if hasattr(args, 'dropout') else 0.0
    question_encoder = HFESMEncoder.init_encoder(args.pretrained_model_cfg,
                                                  projection_dim=args.projection_dim, dropout=dropout, **kwargs)
    ctx_encoder = HFESMEncoder.init_encoder(args.pretrained_model_cfg,
                                             projection_dim=args.projection_dim, dropout=dropout, **kwargs)
    ...
    tensorizer = get_esm_tensorizer(args)
...
def get_esm_tensorizer(args, tokenizer=None):
    if not tokenizer:
        tokenizer = get_esm_tokenizer(args.pretrained_model_cfg, do_lower_case=args.do_lower_case)
    return ESMTensorizer(tokenizer, args.sequence_length)
...
def get_esm_tokenizer(pretrained_cfg_name: str, do_lower_case: bool = True): 
    return EsmTokenizer.from_pretrained(pretrained_cfg_name, do_lower_case=do_lower_case)
...
class HFESMEncoder(EsmModel):

    def __init__(self, config, project_dim: int = 0):
        EsmModel.__init__(self, config)
    ...
    def init_encoder(cls, cfg_name: str, projection_dim: int = 0, dropout: float = 0.1, **kwargs) -> BertModel:
        cfg = AutoConfig.from_pretrained(cfg_name if cfg_name else 'esm2_t33_650M_UR50D')
        if dropout != 0:
    ...
    def forward(self, input_ids: T, token_type_ids: T, attention_mask: T) -> Tuple[T, ...]:
        if self.config.output_hidden_states:
            ...
            hidden_states = None
            x = super().forward(input_ids=input_ids, attention_mask=attention_mask)

        pooled_output = x.last_hidden_state[:, 1, :] # ESM recommends the 1st index
        if self.encode_proj:
            pooled_output = self.encode_proj(pooled_output)
        return x.last_hidden_state, pooled_output, hidden_states
...
class ESMTensorizer(Tensorizer):
    def __init__(self, tokenizer: EsmTokenizer, max_length: int, pad_to_max: bool = True):
    ...
    def text_to_tensor(self, text: str, title: str = None, add_special_tokens: bool = True):
        text = text.strip()
        print('is text upper', text.isupper()) # amino acids should be all upper case
        print('is title none', title is None) # the text field is where the amino acid goes, not the title 
        # tokenizer automatic padding is explicitly disabled since its inconsistent behavior
        token_ids = self.tokenizer.encode(text, add_special_tokens=add_special_tokens, max_length=self.max_length, pad_to_max_length=False, truncation=True)

        seq_len = self.max_length
        if self.pad_to_max and len(token_ids) < seq_len:
            print('todo remove this part')
    ...
    def is_sub_word_id(self, token_id: int):
        exit('unimplemented is_sub_word_id')
        return None
...
class HFBertEncoder(BertModel):
    ...
    def forward(self, input_ids: T, token_type_ids: T, attention_mask: T) -> Tuple[T, ...]:
        if self.config.output_hidden_states:
            ...
            hidden_states = None
            x = super().forward(input_ids=input_ids, token_type_ids=token_type_ids, attention_mask=attention_mask)

        print("TODO, i think ESM recommends this is 1")
